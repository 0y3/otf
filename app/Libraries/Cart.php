...
class Cart
{
    ...
    protected function _insert($items = [])
    {
        ...
        $old_total = isset($this->cartContents[ $rowid ][ 'total' ]) ? (int)$this->cartContents[ $rowid ][ 'total' ] : 0;

        // Re-create the entry, just to make sure our index contains only the data from this submission
        $items[ 'rowid' ] = $rowid;
        $items[ 'qty' ] += $old_quantity;
        $items[ 'total' ] += $old_total;
    ...
    protected function saveCart(): bool
    {
        ...
        foreach ( $this->cartContents as $key => $val ) {
            ...
            if ( ! is_array($val) || ! isset($val[ 'total' ], $val[ 'qty' ]) ) {
                continue;
            }

            $this->cartContents[ 'cart_total' ] +=  $val[ 'total' ] ;
            $this->cartContents[ 'total_items' ] += $val[ 'qty' ];
            $this->cartContents[ $key ][ 'subtotal' ] = $this->cartContents[ $key ][ 'total' ] ;
            // print("<pre>".print_r($this->cartContents,true)."</pre>");
        }
    ...
    protected function saveCart_(): bool
    {
        // Let's add up the individual prices and set the cart sub-total
        $this->cartContents[ 'total_items' ] = $this->cartContents[ 'cart_total' ] = 0;
        foreach ( $this->cartContents as $key => $val ) {
